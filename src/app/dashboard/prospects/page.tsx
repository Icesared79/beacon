...
import { SIGNAL_COLORS, getSignalBadgeColor, BADGE_STYLES } from '@/lib/design-tokens';
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--beacon-text)' }}>Households</h1>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-[9px] border border-beacon-border rounded-lg bg-beacon-surface text-sm text-beacon-text placeholder:text-beacon-text-muted focus:outline-none focus:ring-2 focus:ring-beacon-primary/20"
          />
          {nameSearch && (
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none text-sm border border-beacon-border rounded-lg pl-3 pr-8 py-[9px] bg-beacon-surface text-beacon-text focus:outline-none focus:ring-2 focus:ring-beacon-primary/20 cursor-pointer"
          >
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
          {activeFilters.map((f) => (
            <button
              key={f.key}
              onClick={f.clear}
              className="inline-flex items-center gap-1.5 px-3 py-1 text-[13px] font-medium rounded-full border border-beacon-border bg-beacon-surface text-beacon-text-secondary hover:bg-beacon-surface-alt transition-colors cursor-pointer"
            >
              {f.label}
...
function ProspectRow({ prospect, groupColor }: { prospect: Prospect; groupColor: string }) {
  ...
  const badgeColor = primarySignal ? getSignalBadgeColor(primarySignal) : null;
  const badgeStyle = badgeColor ? BADGE_STYLES[badgeColor] : null;
  ...
  return (
    ...
    >
      <div className="flex items-center px-4 border-b border-beacon-border/50 hover:bg-beacon-surface-alt transition-colors cursor-pointer group" style={{ minHeight: 52 }}>
        {/* Owner + address */}
        <div className="flex-1 min-w-0">
          <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--beacon-text)', lineHeight: 1.5 }} className="truncate">{formatOwnerName(prospect.owner_name)}</p>
          <p style={{ fontSize: 13, color: 'var(--beacon-text-muted)', lineHeight: 1.5 }} className="truncate">{prospect.address}, {prospect.city}, {prospect.state} {prospect.zip}</p>
        </div>

        {/* Primary signal badge + count */}
        <div className="flex items-center gap-2 ml-6 min-w-[140px]">
          {primaryDef && badgeStyle && (
            <span
              className={cn('inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap', badgeStyle.bg, badgeStyle.text)}
            >
              {primaryDef.label}
            </span>
          )}
        ...
        <div className="text-right ml-6 min-w-[100px]">
          {prospect.last_sale_price != null && prospect.last_sale_price <= 1 ? (
            <>
              <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--beacon-text)', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(prospect.assessed_value)}</p>
              <p style={{ fontSize: 11, color: 'var(--beacon-text-muted)' }}>Based on assessed value</p>
            </>
          ) : (
            <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--beacon-text)', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(prospect.estimated_equity || prospect.assessed_value)}</p>
          )}
        </div>
        ...
        <div className="text-right ml-6 min-w-[48px]">
          {days !== null ? (
            <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--beacon-text)', fontVariantNumeric: 'tabular-nums' }}>
